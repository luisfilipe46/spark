...
var listNumbers = List.range(1, 20);System.out.println("""listNumbers  : List[Int] = """ + $show(listNumbers ));$skip(66); 

for (number <- listNumbers) {
if (number%3 == 0)
println(number)
};$skip(34); val res$0 = 

listNumbers.filter(_ % 3 == 0);System.out.println("""res0: List[Int] = """ + $show(res$0))}
