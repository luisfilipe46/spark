...
object LinearRegressionRDD {
  ...
  def main(args: Array[String]) {
    ...
    val squaredAcumulatedError = sc.doubleAccumulator("SquaredErrorAccumulator")
    val numOfPoints = sc.doubleAccumulator("NumberOfPoints")
    
    // Print out the predicted and actual values for each point
    for (prediction <- predictionAndLabel) {
      val error = prediction._1 - prediction._2
      val actualSquaredError = error * error
      
      squaredAcumulatedError.add(actualSquaredError)
      numOfPoints.add(1.0)
      println(prediction)
    }
    println("--------")
    println(squaredAcumulatedError.value)
    println(numOfPoints.value)
    var averageOfSquaredError = squaredAcumulatedError.value / numOfPoints.value
    println(averageOfSquaredError)
  }
}